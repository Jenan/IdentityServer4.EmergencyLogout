        ...
        public async Task<IActionResult> Login(LoginInputModel model, string button)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (_users.ValidateCredentials(model.Username, model.Password))
                {
                    ...
                    // create my claims
                    var claimsIdentity = new ClaimsIdentity(IdentityServerConfig.CookieName);
                    claimsIdentity.AddClaim(new Claim(JwtClaimTypes.Subject, user.SubjectId));
                    claimsIdentity.AddClaim(new Claim(JwtClaimTypes.Name, model.Username));
                   
                    // Create claims principal
                    var claimsPrincipal = new ClaimsPrincipal(claimsIdentity);

                    // issue authentication cookie with subject ID and username
                    await HttpContext.SignInAsync(IdentityServerConfig.CookieName, claimsPrincipal, props);
